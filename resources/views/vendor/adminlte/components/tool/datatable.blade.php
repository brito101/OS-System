...
@push('js')
    ...
    @elseif (isset($withFooter) && $withFooter == 'ticket-payments')
        <script>
            $(() => {
                let {{ $id }} = $('#{{ $id }}').DataTable(
                        {!! substr(json_encode($config), 0, -1) !!},
                        "footerCallback": function(tfoot, data, start, end, display) {
                            ...
                            $(api.column(1).footer()).html('');
                            $(api.column(2).footer()).html('');
                            $(api.column(3).footer()).html(`Pago: ${payed}`);
                            $(api.column(4).footer()).html(`Pendente: ${unpayed}`);
                            $(api.column(5).footer()).html(`Balanço: ${balance}`);
