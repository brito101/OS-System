...
class AlterViewServiceOrderAddEditor extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        SELECT so.id, ac.name as activity, cl.name as client, so.user_id, us.name as collaborator, so.priority, so.deadline, so.status, author as author_id, aut.name as author, so.readiness_date
